...
if __name__ == '__main__':
    ...
    def prediction_to_file(img_idx, Zi, submission_file):
        predictions = Zi.reshape(38,38)
        for j in range(0, predictions.shape[1]):
            for i in range(0, predictions.shape[0]):
                with open(submission_file, 'a') as f:
                    _id = '{:03d}'.format(img_idx)
    ...
    ids = [int(filename.split('_')[1].split('.')[0]) for filename in test_files]
    ids.sort()
