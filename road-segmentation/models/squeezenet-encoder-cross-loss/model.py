...
class Model(AbstractModel):
    ...
    def build_model(self):
        ...
        # final conv layer
        input = tf.layers.conv2d_transpose(inputs=input, filters=2, kernel_size=7, strides=2,
                                                      padding='same', activation=tf.nn.leaky_relu)
        input = tf.layers.conv2d(inputs=input, filters=2, kernel_size=3, strides=1, padding='same')
        predictions = tf.nn.softmax(input, axis=-1)
        self.predictions = tf.cast(tf.expand_dims(tf.argmax(predictions, axis=-1), axis=-1), tf.float32)
        loss_labels = tf.squeeze(self.dataset.labels, axis=-1)
        loss_labels = tf.one_hot(loss_labels, 2)
        print(loss_labels, predictions)

        # Define predictions, train_op, loss
        #self.loss = tf.losses.mean_squared_error(self.labels, self.predictions)
        self.loss = tf.losses.softmax_cross_entropy(loss_labels, predictions)
